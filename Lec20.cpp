...
void addright(Node* root, vector<int>& ans){
    ...
    vector<int>tmp;
    while(curr){
        if(!is_leaf(curr)) tmp.push_back(curr->data);
...
vector<int> boundary(Node* head){
    vector<int>ans;
    if(head==NULL) return ans;
    if(!is_leaf(head)) ans.push_back(head->data);
    addleft(head,ans);
    addleaf(head,ans);
    addright(head,ans);
    return ans;
...
void solve(){
    ...
    vector<int>count=boundary(root);
